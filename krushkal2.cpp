...
void AllConn(Vertex* node, int* arr) {// DFS
    ...
    arr[Index(node->vertex)] = 1;

    Edge* sub = node->Sstart;
    while (sub != NULL) {
        if (arr[Index(sub->ToVertex->vertex)] != 1) {
...
int LoopDetect(Vertex* main) {
    ...
    while (!q1.empty()) {
        ...
        while (sub != NULL) {// traversing sublist
            if (arr[Index(sub->ToVertex->vertex)] == 1 && sub->ToVertex->vertex != output->parent->vertex) {//found the vertex through path
                cout << "loop exist" << endl;
                return(1);
            }
            if (arr[Index(sub->ToVertex->vertex)] == 0)
            {
                loop* relation2 = new loop;
                relation2->parent = output->child;
                relation2->child = sub->ToVertex;
                q1.push(relation2);
                int index = Index(sub->ToVertex->vertex);
                // root is visited
                arr[index] = 1;
            }
            sub = sub->next;
...
int main() {
    ...
    int opt;
    cout << "\t 1: To Enter a Vertex. \n";
    cout << "\t 2: To Insert an Edge. \n";
    cout << "\t 3: To Print Graph. \n";
    cout << "\t 4: To Apply Krushkal. \n";
    cout << "\t 0: To Exit \n";
    cin >> opt;
